...
class Visual_Portfolio_Assets {
    ...
    public static function enqueue_stored_assets( $type = 'script' ) {
        ...
        foreach ( self::$stored_assets[ $type ] as $name => $data ) {
            if ( isset( $data['value'] ) && $data['value'] ) {
                if ( 'script' === $type ) {
                    wp_enqueue_script( $name, '', array(), '2.19.1', true );
                } elseif ( is_string( $data['value'] ) ) {
                    visual_portfolio()->include_template_style( $name, $data['value'], array(), '2.19.1' );
                } else {
                    wp_enqueue_style( $name, '', array(), '2.19.1' );
                }
    ...
    public static function enqueue( $options ) {
        ...
        if ( ! wp_style_is( $controls_css_handle, 'enqueued' ) ) {
            ...
            if ( $dynamic_styles_inline_style ) {
                wp_register_style( $controls_css_handle, false, array(), '2.19.1' );
                ...
                wp_register_script( $controls_css_handle, false, array(), '2.19.1', true );
    ...
    public function register_scripts() {
        ...
        if ( apply_filters( 'vpf_enqueue_plugin_lazysizes', true ) ) {
            wp_register_script( 'lazysizes-config', visual_portfolio()->plugin_url . 'assets/js/lazysizes-cfg.min.js', array(), '2.19.1', true );
            ...
            wp_register_script( 'lazysizes-swiper-duplicates-load', visual_portfolio()->plugin_url . 'assets/js/lazysizes-swiper-duplicates-load.min.js', array(), '2.19.1', true );
        ...
        foreach ( $vp_styles as $name => $data ) {
            wp_register_style( $name, visual_portfolio()->plugin_url . $data[0], $data[1], '2.19.1' );
        ...
        foreach ( $vp_scripts as $name => $data ) {
            wp_register_script( $name, visual_portfolio()->plugin_url . $data[0], $data[1], '2.19.1', true );
        }
